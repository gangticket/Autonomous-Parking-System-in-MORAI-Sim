...
def collision_check(position_parent, yaw_rate, delta_time_step, obstacle_list, Vx):
    ...
    for pos in check_path:
        # Update the position of the car model.
        car_model = Car(pos[0], pos[1], 5.205, 1.915, pos[2])
        corners = car_model.get_corners()

        for obstacle in obstacle_list:
            # Check each corner of the vehicle for collisions with obstacles
            for corner in corners:
                if obstacle.is_inside(corner[0], corner[1]):
                    return True
    return False
