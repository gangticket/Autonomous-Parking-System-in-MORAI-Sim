...
class latticePlanner:
    ...
    def checkObject(self, ref_path, object_data):
        ...
        for obstacle in object_data.obstacle_list:
            for path in ref_path.poses:
                # print("object list check : ", obstacle.position.x)
                # print("reference path check : ", path.pose.position.x)
                dis = sqrt(pow(path.pose.position.x - obstacle.position.x, 2) + pow(path.pose.position.y - obstacle.position.y, 2))                
                print("distance : ", dis)
                if dis < 500 : # 장애물의 좌표값이 지역 경로 상의 좌표값과의 직선거리가 2.35 미만일때 충돌이라 판단.
                    is_crash = True
                    # print("crash detect!!!!!!!!!!!")
                    break
    ...
    def object_callback(self,msg):
        ...
        # print("this is from callback!!", msg)
