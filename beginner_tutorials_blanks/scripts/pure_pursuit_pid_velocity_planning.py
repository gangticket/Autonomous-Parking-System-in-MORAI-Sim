...
class pure_pursuit :
    def __init__(self):
        ...
        self.min_lfd = 5 # 5
        self.max_lfd = 30 # 30
        self.lfd_gain = 0.58 # 1.0
        self.target_velocity = 10

        # control val - I term add
        self.i_gain = 0.3 # 0.0000000000000000003
        self.i_control = 0
        self.i_control2 = 0
        prev_time = 0
    ...
    def calc_pure_pursuit(self,):
        ...
        x = self.global_path.poses[self.current_waypoint].pose.position.x
        y = self.global_path.poses[self.current_waypoint].pose.position.y

        _x = self.current_postion.x
        _y = self.current_postion.y

        error = sqrt(pow(x-_x, 2) + pow(y-_y, 2))
        # error = x-_x, 2) + pow(y-_y, 2)

        self.i_control2 += self.i_gain * error * self.controlTime
        #print("i value : ", self.i_gain * error * self.controlTime)
        # print("i_gain : ", self.i_gain)
        print("error : ", error)
        print("self.controlTime : ", self.controlTime)
        print("i_control2 : ", self.i_control2)


        # steering = atan2((2 * self.vehicle_length * sin(theta)), (self.lfd)) + self.i_control
        # steering = max(-self.max_steering, min(self.max_steering, steering))

        steering = atan2((2 * self.vehicle_length * sin(theta)), (self.lfd))
        print("pure pursuit : ", steering)
        steering = max(-self.max_steering, min(self.max_steering, steering))

        #print("steering : ",steering, ", i_value : ", self.i_control)
